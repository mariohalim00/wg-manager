...
**Feature Branch**: `002-dashboard-ui-polish`  
**Created**: 2026-02-01  
**Status**: Draft  
...
- **FR-002**: The header MUST display interface context on the left and include search, notifications, and user profile controls on the right. Controls use 20px icons, a 240px-wide search input on desktop (full-width on mobile), and 12–16px gaps.
...
- **FR-007**: Visual semantics MUST use two primary palettes (primary and secondary) supported by neutral grays and limited accents. Use Tailwind tokens: primary = blue/indigo range, secondary = emerald/green range, text = gray-100/400/500, borders = gray-700 (dark) and gray-200 (light).
...
- **FR-010**: When required data is missing, the dashboard MUST display the placeholder value "—" and record missing fields in a backlog section within this specification.
...
- **FR-013**: Traffic charts MUST use static, decorative placeholders with a dedicated chart container element and a stable class name (e.g., .traffic-chart) to enable future data binding.
...
- Missing dashboard fields (public key, listening port, subnet) will use the placeholder "—" until data is available.
